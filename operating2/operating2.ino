  ...
  int K3 = -1;
    ...
    // wait for MPU interrupt or extra packet(s) available
     while (!mpuInterrupt && fifoCount < packetSize) {
      rc_read_values();
      int Ox = int(ypr[2] * -180/M_PI); //take the angle from the imu
      int Oy = int(ypr[1] * 180/M_PI);
      int Oz = int(ypr[0] * -180/M_PI);    
      int Oradiox = degres(rc_values[RC_CH1]);
      int Oradioy = degres(rc_values[RC_CH2]);
      int Oradioz = degres (rc_values[RC_CH4]);
      int Xthrotle = degres(rc_values[RC_CH3]);
      int OVRA = degres(rc_values[RC_CH6]);
      int Oservox,Oservoy,Oservoz;
      
      if (abs(rc_values[RC_CH5]-1500)<env || rc_values[RC_CH5]==0){
        Oservox = K1*Ox +90 ;
        if (abs(rc_values[RC_CH5]-1500)<env){Oservoy = K2*Oy + OVRA +90 ;
        Xthrotle = 0;
        }
        else{Oservoy = K2*Oy +90 ;}
        Oservoz = K3*Oz + 90;
          }
      ...
      Serial.print("OX : ");Serial.print(Oservox);Serial.print("    "); myservoRoll.write(Oservox); 
      Serial.print("OY : ");Serial.print(Oservoy);Serial.print("    "); myservoPitch.write(Oservoy); 
      Serial.print("OZ : ");Serial.print(Oservoz);Serial.print("    "); myservoYaw.write(Oservoz); 
      Serial.print("TH : ");Serial.print(Xthrotle);Serial.print("    "); myservoThrotle.write(Xthrotle); 
      Serial.print("SW : ");Serial.print(rc_values[RC_CH5]);Serial.print("    "); 
      Serial.print("OA : ");Serial.print(OVRA);Serial.println("    ");
